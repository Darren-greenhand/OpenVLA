...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # print('traj_len:', traj_len.numpy())

        # with open('./dmf.txt', 'a') as f:
        #     f.write(f'{traj_len}\n')
    ...
    # for traj in tqdm(full_dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None):
    #     actions.append(traj["action"])
    #     proprios.append(traj["proprio"])
    #     num_transitions += traj["action"].shape[0]
    #     num_trajectories += 1
    # print('action shape', actions[0].shape) # kuka: 20,7 , fractal: 64,7


    # actions_np =  np.concatenate(actions,axis=0) # 假设actions是形状为 (N, 7) 的数组
    # print(actions_np.shape)

    # # 提取x, y, z, r, p, y
    # # 假设action的顺序为 [x, y, z, r, p, y, griper_state]
    # x = actions_np[:, 0]
    # y_coord = actions_np[:, 1]  # 避免与yaw（y轴旋转）混淆，命名为 y_coord
    # z = actions_np[:, 2]
    # r = actions_np[:, 3]
    # p = actions_np[:, 4]
    # yaw = actions_np[:, 5]
    ...
    # # 创建保存图像的目录
    # output_dir = name+'_action_histograms'
    ...
    # # 绘制并保存直方图
    # for component, data in action_components.items():
    #     # if 'realman' in name:
    #     #     data*=100
    #     plt.figure(figsize=(8, 6))
    #     plt.hist(data, bins=50, color='blue', alpha=0.7, edgecolor='black')
    #     plt.title(f'{name}-action-{component} hist')
    ...
    #     plt.tight_layout()
    #     # 保存图像
    #     save_path = os.path.join(output_dir, f'{component}_histogram.png')
    #     plt.savefig(save_path)
    #     plt.close()
    #     print(f'已保存 {component} 的直方图到 {save_path}')
    ...
    # # 看看shape
    # def get_actual_shapes(dataset):
    #     for element in dataset.take(1):  # 必须取一个元素来查看其实际形状
    #         actual_shapes = tf.nest.map_structure(lambda x: x.shape, element) #不能用tf.shape
    #         return actual_shapes
    # flattened_shapes = get_actual_shapes(dataset)
    # print('shapes after just build rlds dataset:', flattened_shapes)
    # with open('./shapetest.txt', 'a') as f:
    #     f.write('shapes after just build rlds dataset:'+str(flattened_shapes)+'\n')
    # # 检查数据集个数
    # num_steps = 0
    # for i,step in enumerate(tqdm(dataset.iterator())):
    #     num_steps += 1
    # print('num_steps after just build rlds dataset:', num_steps)
    # with open('./shapetest.txt', 'a') as f:
    #     f.write('num_steps after just build rlds dataset:'+str(num_steps)+'\n')    
    ...
    # # 创建保存图像的目录
    # output_dir = name+'_normal_action_histograms'
    # os.makedirs(output_dir, exist_ok=True)
...
def apply_frame_transforms(
    ...
    *,
    ...
    dataset = dataset.frame_map(
        partial(
            apply_obs_transform,
            partial(obs_transforms.decode, resize_size=resize_size, depth_resize_size=depth_resize_size),
        ),
        num_parallel_calls,
    )
...
def make_combined_dataset(
    ...
    *,
    ...
    dataset: dl.DLataset = datasets[0]
