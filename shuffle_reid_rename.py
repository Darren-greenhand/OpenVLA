...
def process_json(json_file_path, image_base_path, output_json_path):
    data_list = []
    print('加载JSON数据...')
    ...
    for dataset_name, items in dataset_groups.items():
        ...
        for idx, item in enumerate(items):
            ...
            new_full_image_path = os.path.join(image_base_path, image_rel_path_new)
            ...
            if os.path.exists(old_full_image_path):
                if os.path.exists(new_full_image_path):
                    print(f"警告！目标图像文件已存在：{new_full_image_path}")
                    os.remove(new_full_image_path)
                os.rename(old_full_image_path, new_full_image_path)
            else:
...
# 调用函数
json_file_path = '/data/jcy/data/openx_part/llava/origin.json'          # 原始JSON文件路径
...
output_json_path = '/data/jcy/data/openx_part/llava/llavadata.json'       # 输出的JSON文件路径
...
process_json(json_file_path, image_base_path, output_json_path)