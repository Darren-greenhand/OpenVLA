...
from prismatic.vla import get_vla_dataset_and_collator, get_vla_dataset_and_collator_generatellavadata
...
class TrainConfig:
    ...
    data_mix: str = "llava"                                        # Data Mixture to use for training
    # Directory Paths
    data_root_dir: Path = Path(                                     # Path to Open-X dataset directory
        "datasets/open-x-embodiment"
    )
    llavadata_root_dir: Path = Path(                                     # Path to Open-X dataset directory
        "/data/jcy/data/openx_part/llava"
    )
    ...
    hf_token: Union[str, Path] = Path("fake_hf_file/fake_hf")                  # Environment variable or Path to HF Token #Test
...
def train(cfg: TrainConfig) -> None:
    ...
    # llavatokenizer = AutoTokenizer.from_pretrained(
    #         hf_hub_path, model_max_length=self.llm_max_length, token=hf_token, padding_side="right"
    #     )

    overwatch.info(f"Creating VLA Open-X Dataset with Mixture `{cfg.data_mix}`")
    vla_dataset, action_tokenizer, collator = get_vla_dataset_and_collator_generatellavadata(
        cfg.data_root_dir,
        cfg.data_mix,
    ...
    # count = 0
    # for _ in vla_dataset:
    #     count += 1
    #     if count % 1000 == 0:
    #         print("Number of samples in the dataset:", count)
    
    # # 直接结束代码
    # return

# 从training/strategy/base_strategy里面找使用dataset的代码copy出来
    ...
    collator = PassCollator() # 不stack，直接返回instances
    ...
    dataloader = DataLoader(
        ...
        collate_fn=collator,
        num_workers=4,
        # worker_init_fn=self.worker_init_fn, #原本有个set_global_seed的函数，直接注释掉吧
    )

    # Save dataset statistics for de-normalization at inference time
    if overwatch.is_rank_zero():
        save_dataset_statistics(vla_dataset.dataset_statistics, Path('/data/jcy/project/openvla/dataset_statistics'))
        return #看看statistics吧

    cfg.llavadata_root_dir.mkdir(exist_ok=True)
    img_save_dir = cfg.llavadata_root_dir / "images"
    img_save_dir.mkdir(exist_ok=True)

    json_save_dir = cfg.llavadata_root_dir
    json_save_dir.mkdir(exist_ok=True)
    json_save_path = json_save_dir / "origin.json"
    ...
    print("Start generating llavadata....")
    for train_idx, batch in enumerate(tqdm(dataloader)):     
        # print('conversation:', batch[0]["conversation"])

        for i in range(len(batch)):
            id += 1
            img = batch[i]["image"]
            conversation = batch[i]["conversation"]
            dataset_name = batch[i]["dataset_name"].decode('utf-8') # bytes to str，为毛会是bytes的

            if dataset_name not in dataset_count:
                dataset_count[dataset_name] = 0
                group_dir = img_save_dir / dataset_name
                group_dir.mkdir(exist_ok=True)
            dataset_count[dataset_name] += 1

            # 8个进程  3w5，最后245000
            image_dir_id = dataset_count[dataset_name] // 140000
            grouped_img_save_dir = img_save_dir / dataset_name / str(image_dir_id)

            if not grouped_img_save_dir.exists():
                grouped_img_save_dir.mkdir(exist_ok=True)

            unique_suffix = uuid.uuid4().hex


            actual_img_save_path = grouped_img_save_dir / f"{dataset_name}_{unique_suffix}.jpg"

            img.save(actual_img_save_path)

        # 把所有样本存成一个列表 json，每个元素是一个字典，包括image的路径，conversation，id
            with open(json_save_path, "a") as f:
                json.dump({"id": f"{unique_suffix}","image": str(actual_img_save_path), "conversations": conversation}, f)
                f.write("\n")
    ...
    print("Finish generating llavadata.")
